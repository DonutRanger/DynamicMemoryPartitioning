...
public class DR_Test {
    ...
    static Queue<MemoryJob> WaitQueue = new LinkedList();
    ...
    public void FirstFitAlgo(LinkedList<MemoryJob> job, LinkedList<MemoryPartition> memorypartition, Queue<MemoryJob> queue){
        int clock = 0;
        int i;
        ...
        if(!queue.isEmpty()){
            ...
            for(i = 0;i<queue.size();i++){
                ...
                for(int k = 0; k<dynamicpartition.size();k++){
                    if(element.getJobSize() <= dynamicpartition.get(k).getMemorySize() && !dynamicpartition.get(k).getOccupied()){

                        int currentJobNum = element.getJobNum();

                        job.get(currentJobNum).setProcessStatus(true);
                        dynamicpartition.get(k).setJobNum(i);
                        dynamicpartition.get(k).setOccupied(true);
                        dynamicpartition.get(k).fragmentationVal(dynamicpartition.get(k).getMemorySize()-job.get(i).getJobSize());
        ...
        for(i = 0;i<job.size();i++){
            ...
            if(job.get(i).getArrivalTime()==clock && !job.get(i).getProcessStatus() && !job.get(i).getJobDone()){
              ...
              for(int k = 0;k<dynamicpartition.size();k++){ 
                ...
                if(job.get(i).getJobSize() <= dynamicpartition.get(k).getMemorySize() && !dynamicpartition.get(k).getOccupied()){
                    ...
                    System.out.println("Job Processing: " + job.get(i).getJobNum());
                    System.out.println("Memory Block Used: " + dynamicpartition.get(k).getMemorySize());
